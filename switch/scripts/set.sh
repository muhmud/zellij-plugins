...
if [[ "$(list_file_contains "$SWITCH_SESSION_ID" "$SWITCH_SESSION_LIST_FILE")" == "0" ]]; then
  ...
  switch --request add-app --socket-file "$SWITCH_SOCKET_FILE" \
    --app "$SWITCH_APP" --mod "$SWITCH_MOD_KEY" \
    --key "$SWITCH_TAB_KEY" \
    --exec "$SCRIPT_DIR/focus-tab.sh $SWITCH_SESSION_ID"
...
if [[ ! -f "$SWITCH_TAB_PANE_LIST_FILE" ]]; then
  switch --request add-app --socket-file "$SWITCH_SOCKET_FILE" \
    --app "$SWITCH_APP-$TAB_ID" --mod "$SWITCH_PANE_MOD_KEY" \
    --key "$SWITCH_PANE_KEY" \
    --exec "$SCRIPT_DIR/focus-pane.sh $SWITCH_SESSION_ID" || true
